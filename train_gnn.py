...
def train(model:GNN, optimizer:torch.optim.Adam, data:Data, train_loader:NeighborSampler, epoch:int, device:torch.device) -> float:
    model.train()
...
if __name__ == "__main__":
    ...
    for epoch in range(args.epochs):
        loss = train(model, optimizer, data, train_loader, epoch, device)
        train_acc, val_acc, test_acc = test(model, data, subgraph_loader, device)
